...
import { getBody, getBodyParam, getFile, getParam, getParamsAndBody, getQueryParam, getUserPermissions } from "../../core/express/extractors";
...
class PageHandlerClass {
    ...
    public search(...args: HandlerArgs<Query>): Promise<IComicPage[]> {
        return pipeTo(Page.search, getBody<Query>, getUserPermissions)(args);
    }
    ...
    public sort(...args: HandlerArgs<{ newIndex: string, pageId: string }>): Promise<IComicPage[]> {
        return pipeTo(Page.sort, getBodyParam("arcId"), getParam("pageId"), getBodyParam("newIndex"), getUserPermissions)(args);
    }
